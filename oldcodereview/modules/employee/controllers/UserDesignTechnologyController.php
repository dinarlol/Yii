...
class UserDesignTechnologyController extends Controller
{


	private $category_id;
	public function init(){
		$this->category_id = Category::model()->find('LOWER(name)=?',array(strtolower(AkimboNuggetManager::$category_userDesignTechnology)))->id;
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['UserDesign'];
			...
			$this->redirect($this->createAbsoluteUrl('nuggetsCreator/index&hash='.AkimboNuggetManager::$category_userDesignTechnology));
		}
		else{
	...
	{
		$model = UserDesign::model()->findByPk($id);
		if(Yii::app()->user->id == $model->user_id)
		{
			...
			throw new CHttpException(400,'Invalid request. Please do not Rrepeat this request again.');
	}
	...
	{
		$model=new UserDesign('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['UserDesign']))
			$model->attributes=$_GET['UserDesign'];
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='user-design-technology-form')
		{

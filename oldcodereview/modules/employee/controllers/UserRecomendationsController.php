...
class UserRecomendationsController extends Controller
{
	...
	*/
	...
	public function actionNuggetsfield()
	{
		...
		echo $nugget;
		$data = AkimboNuggetManager::getNuggetsBox($nugget,$_POST['UserRecomendations']['user_id']);
	...
	public function actionManage($category_id)
	{
		...
		if(isset($_POST['UserRecomendations']))
		{
			...
			foreach ($_POST['UserRecomendations']['id'] as $id){
			$model = $this->loadModel($id);
			if(isset($_POST[$id])){
				if($model->show !== 1){
					$model->show = 1;
	...
	public function actionIndex()
	{
		...
		$user=Person::model()->findByPk(Yii::app()->user->id);
		//print_r($user);
		//$experiences = $user->userWorkExperiences();
	...
	{
		$model=new UserRecomendations('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['UserRecomendations']))
			$model->attributes=$_GET['UserRecomendations'];
	...
	public function loadModelByCategory($category_id)
	{
		...
		//$model=UserRecomendations::model()->findAll(array('user_ids=?','category_id=?'),array(Yii::app()->user->id,$category_id));
		$model=UserRecomendations::model()->findAll($criteria);
		//print_r($model);
		if($model===null)
	...
	{
		$model=UserRecomendations::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}

...
class Time {
	...
	*/
	...
	public static function dateOnly($dateString = null, $format = 'M Y') {
	...
	*/
	function timeAgoInWords($dateTime, $options = array()) {
		...
		if ($diff > abs($now - strtotime($end))) {
			...
			if (!$backwards) {
				$endString = 'ago';
				if (isset($options['endString'])) {
					$endString = $options['endString'];
					unset($options['endString']);}
					
				$relativeDate = sprintf("%s $endString", $relativeDate);
			}
		}
		return $relativeDate;
	}

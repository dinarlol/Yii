...
<div>
	...
	<div class="span13">
		<h2><?php echo MessageModule::t('Draft'); ?></h2>

		<?php if ($messagesAdapter->data): ?>
			<?php $form = $this->beginWidget('CActiveForm', array(
				'id'=>'message-delete-form',
				'enableAjaxValidation'=>false,
				'action' => $this->createUrl('delete/deleteDraft')
			)); ?>
			
				<fieldset>
			<legend>Save Drafts	</legend>
			...
			<table class="bordered-table zebra-striped">
				...
				<?php foreach ($messagesAdapter->data as $index => $message): ?>
					<tr class="<?php echo $message->isRead ? 'read' : 'unread' ?>">
						<td>
							<?php echo CHtml::checkBox("UserMailBoxDrafts[$index][selected]"); ?>
							<?php echo $form->hiddenField($message,"[$index]id"); ?>
							
						</td>
						<td><a href="<?php echo $this->createUrl('view/draft/', array('message_id' => $message->id)) ?>"><?php echo $message->subject ?></a></td>

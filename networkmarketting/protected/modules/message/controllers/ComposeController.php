	...
	public function actionCompose($id = null) {
		...
		if (Yii::app()->request->getPost('UserMailBoxMails')) {
		  ...
		  $message->attributes = Yii::app()->request->getPost('UserMailBoxMails');
                    ...
                    $to = Users::model()->find("username=?",array($message->receiverUserId));
                    
                    if(empty($to->user_id)){
                        
                        return $this->render(Yii::app()->getModule('message')->viewPath . '/compose', array('model' => $message, 'receiverName' => isset($receiverName) ? $receiverName : null));
                    }
                    
                    $message->receiverUserId = $to->user_id;
                    $message->senderUserId = Yii::app()->user->getId();
			$message->create_date = UtilityManager::getSqlCurrentDate();

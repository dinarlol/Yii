...
class Time {
    ...
    public static function getNiceFromSeconds($escrow_seconds, $count) {

        $escrow_seconds = $escrow_seconds / $count;
        $hours = floor($escrow_seconds / 3600);
        $minutes = floor(($escrow_seconds / 60) % 60);
        $seconds = $escrow_seconds % 60;
        if (!empty($hours)) {
            $escrow_score = "$hours hr $minutes min $seconds seconds";
        } else if (!empty($minutes)) {
            $escrow_score = "$minutes min $seconds seconds";
        } else {
            $escrow_score = "$seconds seconds";
        }
    ...
    /*
     * Returns online bootstrap class text-success if time difference is less then 1 minutes
     * Return idle class text-warning if time is between 1 and 2 minutes
     * Return offline class text-danger if time is greater then 2 minutes
     */

    public static function getLoginStatusClass($dateTime) {
        $now = time();
    ...
    /*
     * Returns online bootstrap if time difference is less then 1 minutes
     * Return idle if time is between 1 and 2 minutes
     * Return offline if time is greater then 2 minutes
     */

    public static function getLoginStatus($dateTime) {
        $now = time();
        $inSeconds = strtotime($dateTime);
        $backwards = ($inSeconds > $now);
    ...
    public static function getDifferenceInSeconds($start_time, $finish_time) {
        $now = strtotime($finish_time);
        $inSeconds = strtotime($start_time);
        $backwards = ($inSeconds > $now);
     ...
     */
    function timeAgoInWords($dateTime, $options = array(), $raw_now = null) {

        if (empty($raw_now)) {
            $now = time();
        } else {
            $now = strtotime($raw_now);
        }
